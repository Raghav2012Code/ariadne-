...
- `GridAnimator` touches the DOM directly and owns the only animation token: every transition (`initializeGrid`, `clearPath`, `clearWalls`, `setAlgorithm/Difficulty/Density`, `runSearch`, `generateMaze`) must go through the store's `cancelAnimation()` (cancels timeline, resets cell styles, aborts the controller); long runs are guarded by `AbortController` — always thread `signal` through and handle `AbortError`.

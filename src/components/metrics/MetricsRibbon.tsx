...
function Stat({ icon, label, value, accent }: { icon: React.ReactNode; label: string; value: string; accent?: string }) {
  return (
    <div className="flex items-center gap-2.5 rounded-md border border-white/[0.07] bg-white/[0.03] px-3 py-1.5 min-w-[118px]">
      <span className="grid place-items-center w-7 h-7 rounded-lg bg-white/[0.05] border border-white/[0.07] text-zinc-300">
        {icon}
      </span>
      <span className="leading-none">
        <span className="block text-[9px] font-bold tracking-[0.1em] uppercase text-zinc-500">{label}</span>
...
export function MetricsRibbon() {
  ...
  return (
    <footer
      id="ribbon"
      className="shrink-0 z-20 border-t border-white/[0.07] bg-[#0A0C10] px-4 lg:px-6 py-2.5"
    >
